...
##############################################################################
# GovPay NOTIFY (Verifica attivazione NOTIFY) - Script di Entrypoint
#
...
log_info "========================================"
log_info "Avvio GovPay NOTIFY Batch Processor"
log_info "========================================"
...
GOVPAY_NOTIFY_MIN_POOL=${GOVPAY_NOTIFY_MIN_POOL:-2}
GOVPAY_NOTIFY_MAX_POOL=${GOVPAY_NOTIFY_MAX_POOL:-10}
    ...
    export SPRING_DATASOURCE_HIKARI_MINIMUM_IDLE="${GOVPAY_NOTIFY_MIN_POOL}"
    export SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE="${GOVPAY_NOTIFY_MAX_POOL}"

    # Export per init_notify_db.sh
...
if [ "${GOVPAY_NOTIFY_POP_DB_SKIP:-TRUE}" != "TRUE" ] && [ -n "${GOVPAY_DB_TYPE}" ]; then
    log_info "Esecuzione script di inizializzazione database..."
    /usr/local/bin/init_notify_db.sh
    if [ $? -ne 0 ]; then
...
# Integrazione GDE disabilitata di default a meno che non venga fornito GOVPAY_NOTIFY_GDE_URL
if [ -n "${GOVPAY_NOTIFY_GDE_URL}" ]; then
    GOVPAY_GDE_ENABLED=true
    GOVPAY_GDE_BASE_URL="${GOVPAY_NOTIFY_GDE_URL}"
...
# Verifica se modalità CRON è abilitata tramite GOVPAY_NOTIFY_BATCH_USA_CRON
...
case "${GOVPAY_NOTIFY_BATCH_USA_CRON,,}" in
    si|yes|1|true)
    ...
    *)
        ...
        INTERVALLO_MINUTI=${GOVPAY_NOTIFY_BATCH_INTERVALLO_CRON:-120}
        SCHEDULER_NOTIFYACQUISITIONJOB_FIXEDDELAYSTRING=$((INTERVALLO_MINUTI * 60 * 1000))

        export SERVER_PORT SCHEDULER_NOTIFYACQUISITIONJOB_FIXEDDELAYSTRING
        log_info "Porta Actuator: ${SERVER_PORT}, Intervallo scheduler: ${INTERVALLO_MINUTI} minuti (${SCHEDULER_NOTIFYACQUISITIONJOB_FIXEDDELAYSTRING}ms)"
        ;;
esac
...
JVM_MEMORY_OPTS="-XX:MaxRAMPercentage=${GOVPAY_NOTIFY_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}"
[ -n "${GOVPAY_NOTIFY_JVM_INITIAL_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:InitialRAMPercentage=${GOVPAY_NOTIFY_JVM_INITIAL_RAM_PERCENTAGE}"
[ -n "${GOVPAY_NOTIFY_JVM_MIN_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MinRAMPercentage=${GOVPAY_NOTIFY_JVM_MIN_RAM_PERCENTAGE}"
[ -n "${GOVPAY_NOTIFY_JVM_MAX_METASPACE_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxMetaspaceSize=${GOVPAY_NOTIFY_JVM_MAX_METASPACE_SIZE}"
[ -n "${GOVPAY_NOTIFY_JVM_MAX_DIRECT_MEMORY_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxDirectMemorySize=${GOVPAY_NOTIFY_JVM_MAX_DIRECT_MEMORY_SIZE}"
...
log_info "Java: MaxRAMPercentage=${GOVPAY_NOTIFY_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}%"
log_info "========================================"
...
JAR_FILE=$(find /opt/govpay-notify -name "*.jar" -type f | head -n 1)

if [ -z "${JAR_FILE}" ]; then
    log_error "Nessun file JAR trovato in /opt/govpay-notify"
    exit 1
fi
